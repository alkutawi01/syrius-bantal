...
**This session's headless browser tool does not reliably fire `IntersectionObserver` callbacks**, even for elements already inside the viewport on load (confirmed by attaching a fresh observer directly and getting no callback after a 2s timeout). `ScrollReveal.tsx`'s fade-in relies entirely on `IntersectionObserver`, so testing it in this tool will make every `[data-reveal]` element look permanently stuck at `opacity: 0` — that is a tooling limitation, not a site bug. Don't "fix" scroll-reveal based on that symptom; verify real behaviour by reading the component logic and CSS instead, or by manually adding the `.is-visible` class in a test script when you need to see the revealed state.
