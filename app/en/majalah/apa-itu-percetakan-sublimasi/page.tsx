...
const steps: [string, string, ReactNode][] = [
  ...
  ['Ink turns to gas', "Heat turns the solid ink directly into gas and opens up the fabric's fine pores.", <svg key="s3" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="7" cy="8" r="1.3" fill="currentColor" /><circle cx="12" cy="6" r="1.3" fill="currentColor" /><circle cx="17" cy="9" r="1.3" fill="currentColor" /><circle cx="9" cy="13" r="1.3" fill="currentColor" /><circle cx="15" cy="14" r="1.3" fill="currentColor" /><circle cx="12" cy="18" r="1.3" fill="currentColor" /></svg>],
  ['Locked in permanently', 'The fabric cools, the pores close again, and the ink stays locked inside for good.', <svg key="s4" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 2l2.2 1.3 2.5-.3 1 2.3 2.3 1-.3 2.5L21 11l-1.3 2.2.3 2.5-2.3 1-1 2.3-2.5-.3L12 20l-2.2-1.3-2.5.3-1-2.3-2.3-1 .3-2.5L3 11l1.3-2.2-.3-2.5 2.3-1 1-2.3 2.5.3L12 2Z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" /><path d="M8.5 12l2.3 2.3 4.7-4.7" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>],
];
  ...
  return <main>
    ...
    <article className="section" id="artikel-en" tabIndex={-1}><div className="container" style={{ maxWidth: 720 }}>
      ...
      <p className="lede" data-reveal style={{ marginTop: 0 }}>
        Sublimation printing is a method that uses heat to transfer ink directly into the structure of the fabric, rather than simply sitting on its surface. It's widely used to print logos, patterns and full-colour designs onto textiles such as cushions, jerseys and sportswear.
      </p>
      ...
      <div data-reveal>
        ...
        <p>At this temperature, the solid ink turns directly into gas without passing through a liquid phase — a process called sublimation. The heat also opens up the fabric's fine pores, allowing the ink gas to penetrate. Once the heat is removed and the fabric cools, the pores close again and the ink becomes permanently locked into the fabric's structure.</p>
      </div>

      <div data-reveal>
        <h2 style={{ color: 'var(--ink)', fontSize: 22, letterSpacing: '-.01em', margin: '40px 0 14px' }}>Why polyester fabric is used</h2>
        <p>This method only works on fabric that contains polyester or a polymer coating, because the sublimation process relies on a chemical reaction between the gaseous ink and polyester's molecular structure. Natural fabrics such as cotton don't have a suitable structure to receive the ink this way, so prints on cotton usually turn out faded or unclear compared with results on polyester.</p>
      </div>

      <div data-reveal>
        <h2 style={{ color: 'var(--ink)', fontSize: 22, letterSpacing: '-.01em', margin: '40px 0 14px' }}>Characteristics of the print</h2>
        <p>Because the ink becomes part of the fabric's fibres rather than a layer on top of the surface, sublimation prints don't crack, peel or fade the way surface-layer prints do (such as standard screen printing). This makes it well suited to full-colour designs and patterns that cover an entire product surface, such as an all-over printed cushion or jersey.</p>
